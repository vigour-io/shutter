          ...
          if (res.statusCode === 200) {
            res.on('data', function (chunk) {
              if (!cancel) {
                ...
                if (separator === '') {
                  // Is the chunk long enough to contain a prefix?
                  if (chunkStr.length >= prefixLength) {
                    // Does the chunk begin with a prefix?
                    if (chunkStr.slice(0, prefixLength) === prefix) {
                      separator += chunkStr
                    ...
                    // Does the chunk correspond to the beginning of a prefix?
                    if (chunkStr === prefix.slice(0, chunkStr.length)) {
                      separator += chunkStr
                    } else {
                      ws.write(chunk)
                    }
                  ...
                  separator += chunkStr
                  // Do we already know that the partial separator starts with a prefix?
                  if (separator.length >= prefixLength) {
                    ...
                    // Is the separator long enough to contain the prefix?
                    if (separator.length >= prefixLength) {
                      // Does the separator start with a prefix?
                      if (separator.slice(0, prefixLength) === prefix) {
                        // Does the separator end with a suffix?
                        if (separator.length > suffixLength &&
                          separator.slice(-(suffixLength)) === suffix) {
                          dst = separator.slice(prefixLength, separator.length - suffixLength)
                          ...
                          // console.log('dst3', dst)
                          ws = fs.createWriteStream(dst)
                          separator = ''
                        }
                      } else {
                        console.log('Writing re-buffered separator')
                        ws.write(new Buffer(separator))
                        separator = ''
                      }
                    }
                  }

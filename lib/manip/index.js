        ...
        var req = http.request(reqOptions, function (res) {
          ...
          if (res.statusCode === 200) {
            res.on('data', function (chunk) {
              // if start or middle of separator
              if (chunk.slice(0, options.separatorPrefixBuf.val.length).equals(options.separatorPrefixBuf.val) ||
                ...
                if (chunk.slice(-(options.separatorErrorSuffixBuf.val.length)).equals(options.separatorErrorSuffixBuf.val)) {
                  ...
                  // if end of separator
                  if (chunk.slice(-(options.separatorSuffixBuf.val.length)).equals(options.separatorSuffixBuf.val)) {
                    var str = separator.toString()
                    var dst = str.substring(options.separatorPrefix.val.length,
                      str.length - options.separatorSuffix.val.length)
                    // log.info('receiving', dst)
                    erroring = false
                    if (ws) {
                      ws.end()
                    }

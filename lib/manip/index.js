  ...
  return imageMagickInstalled(this.config.convertPath.val)
    .then(function (is) {
      if (is) {
        ...
        /* This next section will become obsolete once vigour-js method convert handles empty arrays */
        if (!self.config.manip.val.map) {
          return []
        }
        /* end */
        return Promise.all(self.config.manip.val.map(function (item) {

...
config.items = {
  ...
  invalidRequestMessage: {
    def: 'Invalid request. Make sure you are respecting the sprite maker api (https://github.com/vigour-io/shutter/blob/master/README.md#user-content-api) and that the requested data exists.'
  },
  port:
    { def: 8000,
      env: 'IMG_PORT',
  ...
  verbose: {
    ...
    env: 'IMG_VERBOSE',
    cli: '-v, --verbose',
    desc: 'Logs successful requests'
  },
  remote: {
    def: 'img.vigour.io',
    cli: '--remote <url>',
    desc: 'url for the remote image server to use for image manipulations'
  },
  remotePort: {
    def: 80,
    cli: '--remote-port <nb>',
    desc: 'port on which to reach the remote image server'
  }
}

config.files = { def: null,
  env: 'IMG_CONFIG'
}

...
function Shutter (config) {
  ...
  if (!this.config.manip || !this.config.manip instanceof Array || this.config.manip.length === 0) {
    ...
    //   credentials: new AWS.Credentials(this.config.clients.aws.accessKeyId, this.config.clients.aws.secretAccessKey),
    //   apiVersions: {
    ...
    //   cloudfront: this.config.clients.aws.cloudfront.apiVersion
    // }
    // console.log('this.config.clients.aws', this.config.clients.aws)
    // AWS.config.update({
    //   accessKeyId: this.config.clients.aws.accessKeyId,
    //   secretAccessKey: this.config.clients.aws.secretAccessKey
    // })
    // AWS.config.credentials = new AWS.Credentials(this.config.clients.aws.accessKeyId, this.config.clients.aws.secretAccessKey)

      ...
      fs.writeFile(req.pathToOriginal, req.url, {
        ...
        if (err) {
          ...
          if (fallback) {
            ...
            let secure = /^https/.test(fallback)
            let reqOpts = 'http' + (secure ? 's' : '') + '://' + self.config.IP.val + ':' + self.config.port.val + '/image?' + queryString.stringify(req.query)
            let fallbackReq = (secure ? https : http).request(reqOpts, function (fallbackRes) {

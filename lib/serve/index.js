...
module.exports = exports = function () {
  ...
  // API Status checks
  app.get('/_api/status', function (req, res) {
    res.status(200).send('')
  })
  app.options('*', function (req, res) {
    res.status(200).send('')
  })
  // EOF API Status checks
  ...
  app.get('/invalidate/*'
    ...
    , function (req, res, next) {
      ...
      if (self.config.clients.aws.cloudfront.distributionId) {
        arr.push(awsInvalidate(self.config.clients.aws,
          [req.originalUrl]))
      }
      Promise.all(arr)
  ...
  return new Promise(function (resolve, reject) {
    console.log('app.listen', self.config.server.port, self.config.server.ip)
    handle = app.listen(self.config.server.port, self.config.server.ip, function () {
      log.info('Listening on ' + self.config.server.ip + ':' + self.config.server.port)
      resolve(handle)
    })
  })
}
      ...
      fs.writeFile(req.pathToOriginal, req.url, options, function (err) {
        if (err) {
          ...
          if (req.fallback) {
            ...
            fallbackType = 'domain'
          } else if (self.config.fallbacks.default) {
            fallback = self.config.fallbacks.default
            fallbackType = 'default'
          } else if (self.config.fallbacks instanceof String) {
            fallback = self.config.fallbacks
            fallbackType = 'default'
          }
          if (fallback) {
            req.query.url = fallback
            let reqOpts = 'http' + '://' + self.config.server.ip + ':' + self.config.server.port + '/image?' + queryString.stringify(req.query)

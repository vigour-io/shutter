...
module.exports = exports = function () {
  ...
  return new Promise(function (resolve, reject) {
    var IP = self.config.IP && self.config.IP.val
    var handle = IP
      ? app.listen(self.config.port.val, IP, onListening)
      : app.listen(self.config.port.val, onListening)
    function onListening () {
      log.info('Listening on ' + (IP || 'localhost:') + self.config.port.val)
      resolve(handle)
    }
  })
}

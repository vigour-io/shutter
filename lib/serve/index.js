...
module.exports = exports = function () {
  ...
  app.get('/invalidate/*', function (req, res, next) {
    ...
    paths.out = path.join(self.config.outDir.val, encodeURIComponent(target))
    if (req.query.url) {
    ...
    paths.original = path.join(self.config.originalsPath.val, paths.original)
    Promise.all(fs.unlinkAsync(paths.out + '.jpg'), fs.unlinkAsync(paths.out + '.png'), fs.unlinkAsync(paths.original))
        ...
        return imgManip.effect(
          ...
          prom = prom.then(function () {
            return new Promise(function (resolve, reject) {
              ...
              res.write(self.config.separatorPrefix.val + item.dst)
              rs.on('error', reject)
  ...
  return new Promise(function (resolve, reject) {
    handle = app.listen(self.config.port.val, function () {
      log.info('Listening on port ', self.config.port.val)
      resolve(handle)
    })
  })
}

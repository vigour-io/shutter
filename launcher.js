...
module.exports = exports = function (_options) {
  ...
  app.get('/invalidate/*', function (req, res, next) {
    ...
    paths.out = options.outDir + '/' + encodeURIComponent(target)
    if (req.query.url) {
  ...
  app.get('/:image/:width/:height'
    ...
    , function (req, res, next) {
      ...
      req.pathToOriginal = options.originalsPath + '/' + req.query.url.slice(req.query.url.lastIndexOf('/') + 1)
  ...
  app.get('/image/:id/:width/:height'
    ...
    , function (req, res, next) {
      ...
      req.pathToOriginal = options.originalsPath + '/' + req.params.id
      fs.exists(req.pathToOriginal, function (exists) {
        if (exists) {
          next()
        } else {
